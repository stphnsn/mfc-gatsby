...
function getPostBodyComponentsNoJS(postBodyComponents, pluginOptions) {
  return postBodyComponents.filter(postBodyComponent => {
    ...
    if (
      ...
      (postBodyComponent.props.id === 'gatsby-script-loader' || postBodyComponent.props.id === 'gatsby-chunk-mapping')
    ) {
      return false
    }
    ...
    return (
      pageScripts.find(script => {
        return (
          postBodyComponent.type === 'script' && `${__PATH_PREFIX__}/${script.name}` === postBodyComponent.props.src
        )
      }) === undefined
    )
  })
...
export function onPreRenderHTML(
  { getHeadComponents, pathname, replaceHeadComponents, getPostBodyComponents, replacePostBodyComponents },
  pluginOptions
) {
  if (process.env.NODE_ENV !== 'production' || checkPathExclusion(pathname, pluginOptions)) {
    // During a gatsby development build (gatsby develop) we do nothing.
    return
  }
